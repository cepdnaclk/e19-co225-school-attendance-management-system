...
export default function Home() {
  ...
  {/*// Prepare the data to be sent in the POST request
  const studentData = updatedAttendees.find(attendee => attendee.id === id); // Find the student with the matching ID
  const data = {
    ...studentData,
    markAttendance: status,
    timestamp: new Date().toISOString(),
  ...
  const sendMail = (attendeeId) => {
    axios.post(`http://localhost:8080/${attendeeId}/send-email`)
      .then(() => {
        console.log('Mail sent successfully.');
      })
      .catch((error) => {
